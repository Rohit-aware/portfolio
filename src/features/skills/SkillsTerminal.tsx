    ...
    <section id="skills" aria-labelledby="skills-heading-term" className="section-padding">
      <div className="section-container">
        <RevealWrapper>
          ...
          <div
            className="rounded-2xl overflow-hidden border border-[#2a2a3a]"
            style={{ background: '#0d1117', boxShadow: '0 24px 80px rgba(0,0,0,0.5)' }}
            role="region"
          ...
          >
            ...
            {/* Tab bar */}
            <div className="flex items-end border-b border-[#2a2a3a] bg-[#0d0d14] overflow-x-auto" role="tablist" aria-label="Skill category files">
              {EDITOR_FILES.map(f => (
                <button
                  key={f.name}
                  role="tab"
            ...
            <div className="flex" style={{ minHeight: 340 }}>
              <div className="w-10 shrink-0 bg-[#0d0d14] border-r border-[#2a2a3a] hidden sm:flex flex-col items-center pt-3 gap-4">
                {EDITOR_SIDEBAR_ICONS.map((ic, i) => (
                  <span key={i} className="text-[10px] text-[#4a4a5a] opacity-50" aria-hidden="true">{ic}</span>
                ))}
              </div>
              <div className="flex-1 overflow-y-auto p-4" role="tabpanel" aria-label={`${activeFile.category} skills`}>
            ...
            <div className="flex items-center justify-between px-4 py-1.5 bg-[#0d0d14] border-t border-[#2a2a3a]">
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-[10px] font-mono text-[#4a4a5a]">{group?.skills.length ?? 0} exports</span>
              </div>
            </div>

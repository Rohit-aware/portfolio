...
import {
  EDITOR_FILES,
  EDITOR_SIDEBAR_ICONS,
  SKILL_TIER_ORDER,
  TIER_TOKENS,
  type EditorFile,
} from '@/features/skills/constants/terminal'
...
const SkillsTerminal: React.FC = memo(() => {
  ...
  return (
    <section
      id="skills"
      aria-labelledby="skills-heading-term"
      ...
      <div className="section-container">
        <RevealWrapper>
          <div className="text-center mb-10">
            ...
            <h2 id="skills-heading-term" className="heading-lg text-foreground">
              Technical Stack
            </h2>
          </div>
            ...
            >
              {EDITOR_FILES.map((f) => (
                ...
                >
                  ...
                  {activeFile.name === f.name && (
                    <span
                      className="w-1.5 h-1.5 rounded-full bg-primary ml-1 shrink-0"
                      aria-hidden="true"
                    />
                  )}
                </button>
            ...
            <div className="flex" style={{ minHeight: 260 }}>
              <div className="w-8 sm:w-10 shrink-0 bg-[#0d0d14] border-r border-[#2a2a3a] hidden sm:flex flex-col items-center pt-3 gap-4">
                {EDITOR_SIDEBAR_ICONS.map((ic, i) => (
                  <span
                    key={i}
                    className="text-[10px] text-[#4a4a5a] opacity-50"
                    aria-hidden="true"
                  >
                    {ic}
                  </span>
            ...
            <div className="flex items-center justify-between px-3 sm:px-4 py-1.5 bg-[#0d0d14] border-t border-[#2a2a3a]">
              <div className="flex items-center gap-2 sm:gap-3">
                <span className="text-[10px] font-mono text-[#4a4a5a]">
                  Ln {Math.min(revealed, lineCount)}, Col 1
                </span>
                <span className="text-[10px] font-mono text-[#4a4a5a] hidden xs:inline">
                  UTF-8
                </span>
              </div>
              <div className="flex items-center gap-2 sm:gap-3">
          ...
          <div className="flex flex-wrap justify-center gap-2 sm:gap-3 mt-10 sm:mt-8">
            {SKILL_TIER_ORDER.map((tier) => {
              const count = SKILLS_DATA.flatMap((g) => g.skills).filter(
                (s) => s.tier === tier,
              ).length

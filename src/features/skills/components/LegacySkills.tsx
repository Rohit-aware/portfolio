      ...
      <div className="section-container">
        <RevealWrapper>
          <div className="text-center mb-10">
            ...
            <h2 id="skills-heading" className="heading-lg text-foreground">
              Technical Expertise
            </h2>
          </div>
          ...
          <div className="flex flex-wrap justify-center gap-2 mb-8" role="tablist">
            {SKILLS_DATA.map((g) => (
              <button
                ...
                className={categoryTabV({
                  state: cat === g.category ? 'active' : 'inactive',
                })}
              >
                {g.category}
                ...
                return (
                  <div key={tier} className="mb-5 last:mb-0">
                    <div className="flex items-center gap-2 mb-3">
                      <span
                        className={cn('w-2 h-2 rounded-full', TIER_CFG[tier].dot)}
                        aria-hidden="true"
                      />
                    ...
                    <div className="flex flex-wrap gap-2">
                      {skills.map((s) => (
                        <span key={s.id} className={skillPillV({ tier })}>
                          {s.name}
                        </span>
                      ))}
                    </div>
          ...
          <div className="flex justify-center gap-6 mt-5">
            {TIER_ORDER.map((tier) => (
              ...
              >
                <span
                  className={cn('w-1.5 h-1.5 rounded-full', TIER_CFG[tier].dot)}
                  aria-hidden="true"
                />
                {TIER_CFG[tier].label}
              </div>

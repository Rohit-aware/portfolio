...
const ProjectsSection: React.FC = memo(() => {
  ...
  const handleFilter = useCallback((f: FilterOption): void => {
    setActiveFilter(f)
    const first =
      f === 'All' ? PROJECTS_DATA[0] : PROJECTS_DATA.find((p) => p.category === f)
      ...
      <div className="section-container">
        <RevealWrapper className="mb-8">
          <div className="text-center mb-6">
            ...
            <h2 id="projects-heading" className="heading-lg text-foreground">
              Featured Work
            </h2>
            <p className="text-sm text-muted-foreground mt-2">
              Production applications built at Mypcot Infotech.
            </p>
          </div>
          ...
          >
            {PROJECT_FILTERS.map((f) => (
              <button
                ...
                className={filterPillV({
                  state: activeFilter === f ? 'active' : 'inactive',
                })}
              >
                {f}
              </button>

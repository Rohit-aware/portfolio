...
const Globe3D: React.FC = memo(() => {
  ...
  const getColor = useCallback(
    (isDark: boolean): string => (isDark ? '130, 200, 255' : '14, 100, 170'),
    [],
  )
  ...
  const draw = useCallback((): void => {
    ...
    const cosY = Math.cos(rotY.current),
      sinY = Math.sin(rotY.current)
    const cosX = Math.cos(rotX.current),
      sinX = Math.sin(rotX.current)
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          background:
            'radial-gradient(circle at 40% 35%, hsl(var(--primary)) 0%, transparent 65%)',
          filter: 'blur(40px)',
        }}
      />
